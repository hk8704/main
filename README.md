...
분석단계에서의 조건 중 하나로 결제취소(pay)->마일리지 발급 취소(marketing) 간의 호출은 동기식 일관성을 유지하는 트랜잭션으로 처리하기로 하였다. 
...
시나리오는 결제 취소 시 마일리지발급 취소를 RESTful Request/Response 로 연동하여 구현이 되어있고, 결제 요청이 과도할 경우 CB 를 통하여 장애격리.
